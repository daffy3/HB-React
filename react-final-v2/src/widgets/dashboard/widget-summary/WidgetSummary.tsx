...
interface Props {
    label: string;
    data: AstronomyPhotoOfTodayType | NASAImageAndVideoLibraryType | AstronomyEpicType;
}
...
function WidgetSummary({ label, data }: Props) {
    ...
    const isNASAImageAndVideoLibraryType = (data: any): data is NASAImageAndVideoLibraryType => {
        return (data as NASAImageAndVideoLibraryType).nasa_id !== undefined;
    };

    const isAstronomyEpic = (data: any): data is AstronomyEpicType => {
        return (data as AstronomyEpicType).caption !== undefined;
    };
    ...
    return (
        <Card className={styles.card}>
            ...
            {label === "NASA 이미지 & 비디오 라이브러리" && isNASAImageAndVideoLibraryType(data) && (
                <div className={styles[`card__info-box`]}>
                    <div className={styles[`card__info-box__column`]}>
            ...
            {label === "지구 다색 이미징 카메라" && isAstronomyEpic(data) && (
                <div className={styles[`card__info-box`]}>
                    ...
                    <div className={styles[`card__info-box__column`]}>
                        ...
                        <span className="text-sm">
                            <Link
                                to={`https://epic.gsfc.nasa.gov/archive/natural/${data.date.split(" ")[0].split("-")[0]}/${data.date.split(" ")[0].split("-")[1]}/${data.date.split(" ")[0].split("-")[2]}/png/${data.image}.png`}
                                target="_blank"
